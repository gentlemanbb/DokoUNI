...
<script>
	...
	function GetEventData(areaID)
	{
		...
		for(var i = 1; i <= rowHeight; i++)
		{
			...
			for(var j = 1; j <= 7; j++)
			{
				...
				$(rowID).append('<td id="day' + dayCnt + '" class="calendar_cell"><div class="calendar_day">' + dayCnt + '</div><p onClick="Regist();>[登録]</p></td>');
				var celID = "#day" + dayCnt;

				$(celID).append('<td id="event' + dayCnt + '" class="calendar_event"></td>');
				dayCnt = dayCnt + 1;
			}
		}
		...
		if(eventData != null && eventData.length > 0)
		{
			eventData.forEach(function(value)
			{
				...
				$(dateID).append(
					'<div class="calendar_place_name" style="color:red">' + value.PLACE_NAME + '</div>' +
					'<div class="calendar_event_name">' + value.EVENT_NAME + '</div>'
				);
			});
		}
		return true;
	...
	$(document).ready(function()
	{
		...
		var areaID = GetParam('areaID', url);
