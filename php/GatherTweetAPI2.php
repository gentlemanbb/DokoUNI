<?php
	...
	try
	{
			...
			foreach($tweetResult as $tweet)
			{
				...
				if($updateType == 'INSERT')
				{
					try
					{
						...
						/* ------ お気に入りユーザーにリプライを送る ------ */
						$userIDArray = GetFavoriteUserDataArray('PLACE', $placeID);

						// 配列をクリア
						$notificationUsers = [];
						
						if(is_array($userIDArray) && count($userIDArray) > 0)
						{
							// 通知対象のユーザーがいる場合
							foreach($userIDArray as $replyUserID)
							{
								$replyUserData = GetUserDetailData($replyUserID);
	
								if(strlen($replyUserData['TWITTER']) > 0)
								{
									// 有効なTWITTER IDの場合
									// 配列に追加する
									array_push($notificationUsers, $replyUserData['TWITTER']);
								}
							}
	
							$tweetStr = sprintf(
								'#どこUNI %s さん が %s で UNIをプレイ中です。'
								, $name
								, $placeName
							);
							...
							// 通知対象のＩＤを付け加える
							if(is_array($notificationUsers) && count($notificationUsers) > 0)
							{
								...
								WriteLog('log', sprintf('【GetherTweetAPI】ツイート内容：[%s]', $tweetStr));
								// $tweetResult = Tweet($tweetStr);
							}
							else
							{
								WriteLog('log', '【GetherTweetAPI】ツイート不要');
							}
						}
					}
					catch(Exception $ex)
					{
						$fileStr = $fileStr . sprintf('【GetherTweetAPI】プレイ履歴の登録に失敗しました。' . "\n");
					}
				}
				else
				{
		...
		{
			$fileStr = sprintf('【GetherTweetAPI】ツイートはありませんでした。');
		}
		
		// ツイート内容を書き込む
		WriteLog('log', '【GetherTweetAPI】' . $fileStr);
