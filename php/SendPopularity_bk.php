<?php
    ...
    $query = sprintf("
        ...
        WHERE
            POP.USER_ID = '%s'
        AND
            POP.JOIN_DATE_FROM BETWEEN CURRENT_DATE() AND CURRENT_DATE() + 1
    ", $userID);
    ...
    if($prevPopularityID != NULL){
        ...
        $query = sprintf("
            ...
            VALUES (
                ...
                , CURRENT_DATE()
                , '%s'
            )", $placeID, $userID, $characterID, $RIP, $joinType, $purposeType, $from, $to);
        
        $queryResult = $dbAccess->Insert($query);
        
        $tweetStr = sprintf(
            "#どこUNI %s さん が %s に 「%s, %s」 でチェックインしました。"
             , $playerName
            , $placeName
            , $purposeText
            , $joinText);
